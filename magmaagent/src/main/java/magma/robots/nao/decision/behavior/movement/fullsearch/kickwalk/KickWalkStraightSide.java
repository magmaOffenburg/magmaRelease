 ...
 * @author Klaus Dorer
 */
public class KickWalkStraightSide extends KickWalk implements INaoConstants
{
	private static final String NAME = IBehaviorConstants.KICK_WALK_STRAIGHT_SIDE.BASE_NAME;

	protected static final double MAX_KICK_DISTANCE = 5;
	protected static final float KICK_PRIORITY = 50;

	public static KickWalkStraightSide instance(Side side, IRoboCupThoughtModel thoughtModel, ParameterMap params,
			float opponentMaxDistance, IWalkEstimator walkEstimator)
	{
		...
		KickWalkStraightSide kickWalk = new KickWalkStraightSide(
				side, NAME, thoughtModel, move, distribution, walkEstimator, list, opponentMaxDistance);

		return kickWalk;
	}

	protected KickWalkStraightSide(Side side, String baseName, IRoboCupThoughtModel thoughtModel,
			...
			List<IParameterList> list, float opponentMaxDistance)
	{
		...
		kickDecider = new KickWalkDecider(kickDecider.getKickEstimator(), distribution,
				...
				new Pose2D(-0.0, side == Side.LEFT ? -0.15 : 0.15), new Vector2D(0.002, 0), Angle.ZERO, Angle.ZERO,
				MAX_KICK_DISTANCE, MAX_KICK_DISTANCE, -10, opponentMaxDistance, 0.007f, 0f, 0.01f, 6, false,
				KICK_PRIORITY, //
