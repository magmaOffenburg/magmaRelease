...
public class RoboVizBufferUtil
{
	 ...
	 *
	 * @param group - drawing group name (if null, swaps all buffers)
	 */
	 ...
	 *
	 * @param center - coordinates of circle (x,y)
	 * @param radius - radius of circle
	 * @param thickness - width of circle edge in pixels
	 * @param color - color of circle
	 * @param group - drawing group name
	 */
	...
	public static byte[] newLine(Vector3D a, Vector3D b, float thickness, Color color, String group)
	{
		return newLine(new float[] {(float) a.getX(), (float) a.getY(), (float) a.getZ()},
				new float[] {(float) b.getX(), (float) b.getY(), (float) b.getZ()}, thickness, color, group);
	}
	 ...
	 *
	 * @param p - coordinates of point (x,y,z)
	 ...
	 *
	 * @param p - coordinates of sphere center (x,y,z)
	...
	public static byte[] concat(byte[]... bytes)
	{
		...
		byte[] combined = bytes[0];
		for (int i = 1; i < bytes.length; i++) {
			combined = concat(combined, bytes[i]);
		}
		return combined;
	}
}
